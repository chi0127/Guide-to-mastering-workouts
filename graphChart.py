...
class Graph(tk.Frame):
    ...
    def __init__(self,root,user):
        ...
        def search():
            ...
            if db.showDataBySearchDate(self.sdate,self.edate, user) != None:
                res=db.showDataBySearchDate(self.sdate,self.edate, user)
                #date2, username, squat_counter, glutebridge_counter, lunge_counter = db.showDataBySearchDate(self.sdate,self.edate, user)
                for i in range(len(res)):
                    calSqu=Decimal(0.32 * res[i][2]).quantize(Decimal('.00'), ROUND_HALF_UP)
                    calBri=Decimal(0.3 * res[i][3]).quantize(Decimal('.00'), ROUND_HALF_UP)
                    calLun=Decimal(0.5 * res[i][4]).quantize(Decimal('.00'), ROUND_HALF_UP)
                    calToday = calSqu + calBri + calLun
                    self.treeview.insert(parent='', index='end', iid=i, text=res[i][0], values=(res[i][2], res[i][3], res[i][4], calToday))
