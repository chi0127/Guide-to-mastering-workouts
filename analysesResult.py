...
class Aanalyses(tk.Frame):
    ...
    def yearSearch(self):
        ...
        def search():
            ...
            year=entryY.get()
            res=db.showDataBySearchYear(year, self.user)
            try:
                y=int(year)
                msg["text"]=""
            except:
            ...
            if res != None:
                ...
                for d in data["month"]:
                    total.remove(int(d[-2:]))
                for t in total:
                    for k in data.keys():
                        if k=="month": data[k].insert(t-1,f"{year}-{t:02}")
                        else: data[k].insert(t-1,0)
                ...
                #日期設定
                f_plot.set_xticklabels( [str(int(d[-2:]))+"月" for d in m1_t['month']], color='white' )
    ...
    def monthSearch(self):
        ...
        def search():
            ...
            try:
                y=int(year)
                m=int(month)
                res=db.showDataBySearchMonth(year, month, self.user)
            ...
            if res != None:
                for i in range(len(res)):
                    ...
                    data["弓步蹲"].append(int(lunge))
                    data["熱量"].append(float(cal))
                total=[t for t in range(1,self.monthTotalDay[month]+1)]
                for d in data["date"]:
                    total.remove(int(d[-2:]))
                for t in total:
                    for k in data.keys():
                        if k=="date": data[k].insert(t-1,f"{year}-{month}-{t:02}")
                        else: data[k].insert(t-1,0)
                ...
                f_plot.set_xticklabels( [int(d[-2:]) for d in m1_t['date']], color='white' )
    ...
    def weekSearch(self):
        ...
        def search(s):
            ...
            try:
                ...
                res=db.showDataBySearchWeek(str(sdate), str(edate), self.user)
                msg["text"]=""
            except:
                msg["text"]="*請輸入有效數值!"
                return
